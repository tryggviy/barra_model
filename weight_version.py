...
def mainfunc(global_lock, read_path_input, read_path_spec,target_date_rt, target_wgt_dt, futures_pos, \
    ...
    if index35_test == 'p1f0' or ic_pos == if_pos == ih_pos == 0:
        ...
        if sum(etfdict.values()) != 0:
            total_positive = total_share_value+sum(etfdict.values())
            total_negetive = 0.00000000000000000000000000000001
            futures_table = {'IC':zz500,'IF':hs300,'IH':sz50}

            wgt_CH1['weight'] = wgt_CH1.weight * (total_share_value / total_positive)
            ...
            for fts in etfdict:
                ...
                if etfdict[fts]>0:
                    result_df['weight'] = (reuslt_wigt/total_positive) * result_df.weight

                elif etfdict[fts]<0:
                    result_df['weight'] = -(reuslt_wigt/total_negetive) * result_df.weight
            ...
            daily_wigt_dist = {'Portfolio':(total_share_value / total_positive), 'IH':sum(etfdict.values()),'IC':0,'IF':0}
        ...
        total_positive = total_share_value + sum(etfdict.values())
        total_negetive = 0.000000000000000000000000001

        for ft in [ic_weight,if_weight,ih_weight]:
            if ft > 0:
                total_positive += ft
            elif ft < 0:
                total_negetive += ft
            else:
                print('There are futures with 0 positions on '+target_wgt_dt)
        ...
        for fts in futures_dict:
            ...
            if reuslt_wigt>0:
                result_df['weight'] = (reuslt_wigt/total_positive) * result_df.weight
                daily_wigt_dist[fts] = (reuslt_wigt/total_positive)
            elif reuslt_wigt<0:
                result_df['weight'] = -(reuslt_wigt/total_negetive) * result_df.weight
                daily_wigt_dist[fts] = -(reuslt_wigt/total_negetive)
            elif reuslt_wigt==0:
                result_df['weight'] = 0 * result_df.weight
        ...
        shares_final_sum = sum(wgt_CH1.weight)
        ...
        final_wights = idxweights_table
        allsum = sum(final_wights.weight)

        nanlist.extend(nanlisths)
        nanlist.extend(nanlistzz)
        nanlist.extend(nanlistsz)

        daily_wigt_dist['Portfolio'] = (total_share_value/total_positive)
        ...
        hs300['weight'] = (if_weight/future_sum) * hs300.weight
        zz500['weight'] = (ic_weight/future_sum) * zz500.weight
        sz50['weight'] = (ih_weight/future_sum) * sz50.weight
        ...
        daily_wigt_dist = {'Portfolio':0,'IC':(ic_weight/future_sum),'IF':(if_weight/future_sum),'IH':(ih_weight/future_sum)}
    else:
