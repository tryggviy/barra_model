...
def mainf(read_start,read_end,sufx,index_mode, risk_free_rate,read_path_data,read_path_spec):
    ...
    factor_types_return = [(100*sum(x['Return'][:10]),100*sum(x['Return'][10:]), y*100) for x, y in zip(daily_contributions_table_common.values(), dlytotal_specrt_period_common.values())]
    style_factors = [x[0] for x in factor_types_return]
    industry_factors = [x[1] for x in factor_types_return]
    assetslct_factors = [x[2] for x in factor_types_return]
