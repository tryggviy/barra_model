...
def expma(aseries, ratio):
    if len(aseries) == 1:
        ...
        return ratio * aseries[-1] + (1-ratio) * expma(aseries[:-1], ratio)
...
def mainfunc(global_lock, read_path_input, read_path_spec,target_date_rt, target_wgt_dt, futures_pos, \
    ...
    CNE5S_100_Asset_LSR = read_csv(read_path_input + 'CNE5S_100_Asset_LSR.' + target_date_rt, delimiter='|',
                                      header=2).dropna(axis=0)
    CNE5S_100_Asset_LSR.rename(columns={'!Barrid': 'Barrid'}, inplace=True)

    wigts_pairs = final_wights_pair.copy()
    dlytotal_specrt = 0
    spec_risks = len(wigts_pairs) * [0]
    count = 0

    for pair in wigts_pairs:
        try:
            spec_rt = float(
                CNE5_100_Asset_DlySpecRet[CNE5_100_Asset_DlySpecRet.Barrid == pair[0]].SpecificReturn.values[0])
            spec_rk = float(CNE5S_100_Asset_LSR[CNE5S_100_Asset_LSR.Barrid == pair[0]].RootSpecificRisk.values[0])
        except:
            print(pair[0] + ' ' + target_date_rt + ' empty specific return')
            spec_risks[count] = 0
        else:
            dlytotal_specrt += 0.01 * spec_rt * pair[1]
            spec_risks[count] = spec_rk

        count += 1
    # Amendation to the factor returns column
    # Get the factor list name

    covmatx = read_csv(read_path_input + 'CNE5S' + target_date_rt[2:] + '.COV', header=2).dropna(axis=1).iloc[:, 1:].values

    wigts = array(getwigt(wigts_pairs))
    ...
    XFXt = dot(dot(xjk_matrix, covmatx), xjk_matrix.T)

    portfolio_risk = sqrt(dot(dot(wigts, (XFXt+spec_risks)), wigts))
    ...
    global_lock.acquire()
    print('xxxxxxxxxxxxxxxxxx' + str(sqrt(portfolio_risk)) + '\nxxxxxxxxxxxxxxxxxxxx')
    ...
    return (return_contribution,final_stocks_weight,wgted_exp_df,dlytotal_specrt,nanlist,daily_wigt_dist, portfolio_risk)
...
def period_accumalate(global_lock, daily_contributions,daily_weights,exposure_matrix_daily,dlytotal_specrt_dict,nandict,daily_wigt_dist,risk_change_dict,\
    ...
    for ii in range(1,len(trade_dates_list)):
        ...
        daily_wigt_dist[dates],risk_change_dict[dates] = mainfunc(global_lock, read_path_input, read_path_spec, dates, wigt_date, futures_pos, \
...
def mainf(read_start,read_end,sufx,index_mode, risk_free_rate,read_path_data,read_path_spec):
    ...
    risk_change_common = manager.dict()
    p_list = []
    for ix in (date_dict):
        ...
        p=Process(target=period_accumalate,args=(global_lock,daily_contributions_table_common,daily_weights_pairs_common,\
            exposure_matrix_daily_table_common,dlytotal_specrt_period_common,nandict_common,daily_wigt_dist_common,risk_change_common,\
    ...
    risk_change_common = dict(sorted(risk_change_common.items(), \
            key=lambda risk_change_common: dt.datetime.strptime((risk_change_common[0]), '%Y%m%d')))
    ...
    axiss = list(daily_contributions_table_common.keys())
    ...
    plt.xticks(range(len(axiss)),axiss)
    first_legend = plt.legend(handles=[line1,line2,line3], loc=1)

    plt.subplot(2, 1, 2)
    plt.title("Risk Trend")
    plt.plot(risk_change_common.values())
    plt.xticks(range(len(axiss)),axiss)
    expma_predict = expma(list(risk_change_common.values()),0.97)
    print("predicted risk for the next period is "+str(expma_predict))
    plt.show()

    return result_summary

if __name__ == "__main__":
    result_summary = mainf('20180228','20180201','jxhy','p1f1',0.035,'D:/datasets/','D:/jupyter_work/jxhy_info/')
